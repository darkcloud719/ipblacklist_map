...
def generate_ip_count():
	with open('Spamhaus_drop_ip_source.txt','r') as f:
...
def insert_ip_count_to_influxdb(iplist):
	...
	print(date)
	CMD = "curl -i -XPOST 'http://localhost:8086/write?db=mydb&precision=s' --data-binary 'spamhaus_ip_count,date={} count={} {}'".format(date,count,timestamp)
	os.system(CMD)
...
def write_new_md5(new_md5):
	with open('Spamhaus_drop_ip_volume_md5.txt','w') as f:
		f.write(new_md5)

def write_new_data(new_data):
	with open('Spamhaus_drop_ip_source.txt','w') as f:
		f.write(new_data)

def compare_md5():
	new_md5,new_data = get_spamhaus_web_md5()
	with open('Spamhaus_drop_ip_volume_md5.txt','r') as f:
		old_md5 = f.read()
		if old_md5 == new_md5:
			return False
		else:
